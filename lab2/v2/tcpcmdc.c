...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        while (!alarm_sent_off) {
            n = read(sock_fd, response + total_read, BUFF_LENGTH - total_read - 1);
            if (n == -1) {
                if (errno == EINTR) {
            ...
            // Check if we've received all data (server closes write end after command completes)
            // We can detect this by checking if read would block
            char peek;
            ssize_t p = recv(sock_fd, &peek, 1, MSG_DONTWAIT | MSG_PEEK);
            if (p == 0) {
                break; // server closed write end
            } else if (p == -1) {
        ...
        if (!alarm_sent_off && total_read > 0) {
