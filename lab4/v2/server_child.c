...
void run_child_session(int session_idx, int client_sockfd, char *secret) {
    ...
    int udp_client_sockfd = socket(AF_INET, SOCK_DGRAM, 0);
    ...
    for (int i = 0; i < MAX_BIND_ATTEMPTS; i++) { // you can try up to 100 ports
        udp_client_addr.sin_port = htons(client_port);
    ...
    int udp_dest_sockfd = socket(AF_INET, SOCK_DGRAM, 0);
    ...
    struct sockaddr_in udp_tunnel_addr;
    memset(&udp_tunnel_addr, 0, sizeof(udp_tunnel_addr));
    udp_tunnel_addr.sin_family = AF_INET;
    udp_tunnel_addr.sin_addr.s_addr = INADDR_ANY;

    int tunnel_port = BASE_TUNNEL_PORT;
    bind_success = 0;
    for (int i = 0; i < MAX_BIND_ATTEMPTS; i++) {
        udp_tunnel_addr.sin_port = htons(tunnel_port);
    ...
    dest_sockaddr.sin_addr.s_addr = forwardtab[session_idx].destaddr;
    dest_sockaddr.sin_port = htons(forwardtab[session_idx].destpt);

    char dest_str[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &forwardtab[session_idx].destaddr, dest_str, INET_ADDRSTRLEN);
    ...
    while (1) {
        ...
        if (FD_ISSET(client_sockfd, &readfds)) {
            ...
            if (n == 6 && strncmp(term_secret, secret, 6) == 0) {
                ...
                close(udp_client_sockfd);
                close(udp_dest_sockfd);
                forwardtab[session_idx].sourceaddr = 0;
                exit(EXIT_SUCCESS);
            } else if (n == 0) {
        ...
        if (FD_ISSET(udp_client_sockfd, &readfds)) {
            ...
            if (n > 0) {
                ...
                if (forwardtab[session_idx].sourcept == 0) {
                    forwardtab[session_idx].sourcept = ntohs(from_addr.sin_port);
                }
        ...
        if (FD_ISSET(udp_dest_sockfd, &readfds)) {
            ...
            if (n > 0) {
                ...
                client_dest.sin_addr.s_addr = forwardtab[session_idx].sourceaddr;
                client_dest.sin_port = htons(forwardtab[session_idx].sourcept);
